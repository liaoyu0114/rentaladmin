<template>
  <div><br />
    <form>
      ...
      <p>出租方：<span>{hetong.chuzu}</span>（以下简称甲方）；</p>　　　
      <p>承租方：<span >{hetong.zuke}</span>（以下简称乙方）。　</p>　
      <p>租赁房屋地址：<span>{hetong.address}</span>　</p>　
      <p>依据《中华人民共和国合同法》有关条款，甲乙双方经过充分协商，就乙方租赁甲方房屋事宜达成如下协议：</p>
      <p>一、乙方租赁甲方房屋，以下简称租赁房屋。</p>
      <p>二、起租时间：<span>{hetong.fromdate}</span>，退租时间：<span>{hetong.todate}</span>。
        在合同签订之日，乙方需向甲方支付房屋租金￥<span>${hetong.price}</span>。如续租，请提前3天通知甲方，并重新办理租赁手续。<br /></p>　　　　
      <p>三、违约处理：</p>　
      <p>甲方违约：</p>　
      ...
      <div class="button">
        ...
        <input class="btn btn6" value="打印合同" type="button">
